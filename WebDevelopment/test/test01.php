...
<html lang="en">
  ...
  <body>
    <?php
      ...
      if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($_POST["password"])) {
          ...
          if(!$uppercase || !$lowercase || !$number || !$specialChars || strlen($password) < 10) {
            $passwordErr = "at least 10 chars, one upper case letter, one number, and one special character";
          }
        }
        ...
        if (empty($_POST["fName"])){
          ...
          $fName = test_input($_POST["fName"]);
           if (!preg_match("/^(\S[A-Z]{1,10})$/", $fName)){
             $fNameErr = "up to 10 chars, only letters [A-Z], with no space";
          }
        }
        ...
        if (empty($_POST["lName"])){
          ...
          if (!preg_match("/^([A-Z]{1,15})$/", $lName)) {
            $lNameErr = "up to 15 chars, only letters [A-Z], with no more than one blank space";
          }
        }
        ...
        if (empty($_POST["phone"])){
          ...
          if (!preg_match("/^(\++[1-9]{1,15})$/", $phone)){
            $phoneErr = "a ‘+’ followed by up to 15 digits.";
          }
        }
        ...
        if (empty($_POST["postcode"])) {
          ...
          if (!preg_match("/^([A-Z]+[0-9]+[A-Z]+\s+[0-9]+[A-Z]+[0-9])$/", $postcode)) {
            $postcodeErr = "1st , 3 rd and 5th char is a capital letter and 2nd, 4th and 6th char is a digit [0-9]";
          }
        }
        ...
        if (empty($_POST["country"])) {
          ...
          if (!preg_match("/^([A-Z]+[a-z]{4,15})$/", $country)) {
            $countryErr = "must start with uppercase letter only, between 4 to 15 characters";
          }
        }
      }
      ...
      $user = array('email', 'password', 'fName', 'lName', 'phone', 'address', 'addressOptional', 'town', 'region', 'postcode', 'country');
      $BowlersFile = "user.txt";
      $NewBowler = "";
      foreach ($user as $item) {
        if (isset($_POST[$item])) {
          $personInfo = $_POST[$item];
    ...
    <div>
      ...
      <form action="test01.php" method="post" enctype="application/x-www-form-urlencoded">
        ...
        <span></span><input type="text" name="addressOptional"><span class="errMsg"></span><br>
